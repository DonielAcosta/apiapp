...
class Fotos extends Controller {
	...
	function dataedit($id=NULL){
		...
		$tipo =$this->datasis->traevalor('SINVIMG','Especifica el tipo de imagenes jpg o png');
		if(empty($tipo)) $tipo='jpg';
		$tipo = strtolower($tipo);

		$edit->foto = new uploadField('Imagen', 'nombre');
		$edit->foto->rule          = 'required';
		$edit->foto->upload_path   = $this->upload_path;
		$edit->foto->allowed_types = $tipo;
		$edit->foto->delete_file   =false;
		//$edit-> \foto->append('Solo imagenes JPG');
		$edit->foto->file_name = url_title($codigo).'_.'.$tipo;
	...
	function _pre_insert($do){
		...
		$thpath=new Path();
		$thpath->setPath($_SERVER['DOCUMENT_ROOT']);
		$thpath->append($this->upload_path);
		$thpath->append('th_'.$nombre);
		if(file_exists($thpath->getPath())){
			unlink($thpath->getPath());
		}

		$dbcodigo= $this->db->escape($codigos[0]);
		$id=$this->datasis->dameval("SELECT id FROM sinv WHERE codigo=${dbcodigo}");
		...
		foreach($codigos as $codigo){
			if($c){
				...
				$this->db->query($mSQL, array($id, $codigo, $nombre,$this->upload_path,$do->get('comentario')));
			}else{
				$c=true;
			}
		}
	...
	function principal($codigo){
		...
		$dbcodigo=$this->db->escape($codigo);
		$check=$this->datasis->dameval("SELECT COUNT(*) FROM sinvfot WHERE codigo=${dbcodigo} and principal='S'");
		if ($check > 0){
			$mSQL_1=$this->db->query("SELECT id FROM sinvfot WHERE codigo=${dbcodigo} AND principal='S'");
			$row = $mSQL_1->row();
			$ids =$row->id;
	...
	function _post_delete($do){
		...
		$check=$this->datasis->dameval("SELECT COUNT(*) FROM sinvfot WHERE nombre=${dbnombre}");
		if($check<=0){
			$path=new Path();
			$path->setPath($_SERVER['DOCUMENT_ROOT']);
			$path->append($this->upload_path);
	...
	function traerfotos($sucu=null){
		...
		$query  = $this->db->query("SELECT * FROM sucu WHERE codigo=${dbsucu}");
		$msg='';
		if ($query->num_rows() > 0){
			$row = $query->row();
